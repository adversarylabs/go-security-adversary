#!/usr/bin/env node
import { createRequire as __goSecurityCreateRequire } from 'node:module'; const require = __goSecurityCreateRequire(import.meta.url);
...
var domain = {
  ...
  analyze(file) {
    const jwt = /\bjwt\.(?:Parse|ParseWithClaims)\s*\(/.test(file.current) && !/(WithValidMethods|ValidMethods)/.test(file.current) ? contentSignal(file, "go-security.jwt-validation", /\bjwt\.(?:Parse|ParseWithClaims)\s*\(/, "JWT parsing has no visible accepted-algorithm constraint.") : [];
    return {
      signals: [
...
function pathTraversalSignals(file) {
  ...
  const hasConfinement = /filepath\.IsLocal\s*\(/.test(source) || /securejoin\.(?:SecureJoin|SecureJoinVFS)\s*\(/.test(source) || /\bos\.(?:OpenRoot|Root)\b/.test(source) || // Clean + HasPrefix / strings.HasPrefix confinement pattern
  /filepath\.Clean\s*\(/.test(source) && /(?:strings\.HasPrefix|filepath\.HasPrefix)\s*\(/.test(source);
  ...
  const joinAssign = /\b([A-Za-z_]\w*)\s*:?=\s*(?:filepath|path)\.Join\s*\(([^)]*)\)/g;
  const signals = [];
  let match;
  while ((match = joinAssign.exec(source)) !== null) {
    const varName = match[1];
    const args2 = match[2] ?? "";
    const hasDynamicArg = args2.split(",").map((a) => a.trim()).some((a) => a.length > 0 && !/^[`"']/.test(a));
...
function zipSlipSignals(file) {
  if (file.path.endsWith("_test.go")) return [];
  const source = file.current;
  const hasArchive = /\bzip\.(?:OpenReader|NewReader)\b/.test(source) || /\btar\.(?:NewReader|Reader)\b/.test(source) || /archive\/(?:zip|tar)/.test(source);
  if (!hasArchive) return [];
  const joinsEntry = /(?:filepath|path)\.Join\s*\([^)]*\.(?:Name|FileInfo\(\)\.Name)/.test(source) || /(?:filepath|path)\.Join\s*\([^,]+,\s*(?:f|file|hdr|header|entry)\.Name\b/.test(source) || /(?:filepath|path)\.Join\s*\([^,]+,\s*\w+\.Name\b/.test(source);
  if (!joinsEntry) {
    if (!/\.Name\b/.test(source)) return [];
  }
  const hasConfinement = /filepath\.IsLocal\s*\(/.test(source) || /securejoin\.(?:SecureJoin|SecureJoinVFS)\s*\(/.test(source) || /\bfilepath\.Clean\s*\(/.test(source) && /(?:strings\.HasPrefix|filepath\.HasPrefix)\s*\(/.test(source) || /\bos\.(?:OpenRoot|Root)\b/.test(source);
  if (hasConfinement) return [];
  const extracts = /\b(?:os\.(?:OpenFile|Create|MkdirAll|WriteFile)|ioutil\.WriteFile|io\.Copy)\s*\(/.test(source) || joinsEntry;
  if (!extracts) return [];
  return lineSignals(
    file,
...
function receiverBinding(node, source) {
  const text2 = sourceText(node, source).replace(/^\(|\)$/g, "").trim();
  const match = text2.match(/^([A-Za-z_]\w*)\s+\*?([A-Za-z_]\w*)$/);
...
function isNegativeSelfCondition(text2, helper, context) {
  return text2 === `!${helper}(${context})` || text2 === `${helper}(${context})==false` || text2 === `false==${helper}(${context})`;
}
function isPositiveSelfCondition(text2, helper, context) {
  return text2 === `${helper}(${context})` || text2 === `${helper}(${context})==true` || text2 === `true==${helper}(${context})`;
}
...
function firstHTTPExposureSink(fn, calls, flow, source) {
  ...
  for (const call of calls.filter(afterHTTP)) {
    ...
    const text2 = sourceText(args2, source);
    if (referencesIdentifier(text2, flow.error) && !identifierReassignedBetween(fn, flow.error, flow.httpNode.endIndex, call.startIndex, source) || referencesRequestURL(text2, flow)) return call;
  }
...
function expressionUsesURL(node, url, source) {
  ...
  return new RegExp(`^(?:${escaped}|${escaped}\\.String\\(\\)|${escaped}\\.RequestURI\\(\\))$`).test(text2);
}
function referencesIdentifier(text2, identifier) {
  return new RegExp(`\\b${escapeRegExp2(identifier)}\\b`).test(text2);
}
function referencesRequestURL(text2, flow) {
  if (flow.request !== void 0 && new RegExp(`\\b${escapeRegExp2(flow.request)}\\.URL\\b`).test(text2)) return true;
  return new RegExp(`\\b${escapeRegExp2(flow.url)}(?:\\.String\\(\\)|\\b)`).test(text2);
}
function exposureDescription(sink, error, source) {
  const text2 = sourceText(sink, source);
  if (sink.type === "return_statement") return `returned HTTP error ${error}`;
  if (referencesIdentifier(text2, error)) return `logged or wrapped HTTP error ${error}`;
...
function isHTTPSetCookie(callable, aliases, source) {
  const text2 = sourceText(callable, source).replace(/\s/g, "");
  return [...aliases].some((alias) => text2 === `${alias}.SetCookie`);
}
...
function findHTTPCookieLiteral(node, httpAliases, source) {
  ...
  return candidates.find((candidate) => {
    ...
    const text2 = sourceText(type, source).replace(/\s/g, "");
    return [...httpAliases].some((alias) => text2 === `${alias}.Cookie`);
  });
}
...
import { join as join5, posix, relative as relative3, sep as sep2 } from "node:path";
...
async function siblingFamilySources(ctx, files, remainingFileBudget) {
  ...
  for (const family of [...families].sort()) {
    ...
    for (const packageEntry of packageEntries.sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      const packagePath = join5(familyPath, packageEntry.name);
      let sourceEntries;
      try {
      ...
      for (const sourceEntry of sourceEntries.sort((left, right) => left.name.localeCompare(right.name))) {
        ...
        try {
          const absolutePath = await safeExistingPath(repoRoot, join5(packagePath, sourceEntry.name));
          ...
          const current = await readFile6(absolutePath, "utf8");
...
async function safeFamilyPath(repoRoot, family) {
  return safeExistingPath(repoRoot, join5(repoRoot, ...family.split("/")));
}
...
async function attachImportNavigation(ctx, analysis) {
  ...
  for (const signal of analysis.signals) {
    ...
    const key = `go-security.navigation.importers:${signal.path}:${signal.ruleId}`;
    if (seen.has(key)) {
      continue;
    }
    seen.add(key);
